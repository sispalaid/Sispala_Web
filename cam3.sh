...
cd "$(dirname "$0")" || exit
mkdir -p Streams/cam3 Rekaman/cam3
ffmpeg -hide_banner -y -hwaccel vaapi -hwaccel_device /dev/dri/renderD128 -hwaccel_output_format vaapi \
-rtsp_transport tcp -i "rtsp://admin:sispala-RC3@192.168.0.11:555/stream1" \
...
-hls_segment_filename "Streams/cam3/seg%03d.ts" "Streams/cam3/index.m3u8" \
-c:v copy -f segment -segment_time 60 -strftime 1 -reset_timestamps 1 "Rekaman/cam3/%Y-%m-%d_%H-%M.mp4"
sleep 5
exec "$0"
