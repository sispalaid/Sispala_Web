...
TIMEOUT="${CAM2_TIMEOUT:-15}"
RTSP_TRANSPORT="${CAM2_RTSP_TRANSPORT:-tcp}"
OUT_RES=""
...
if (timeout 1 bash -c '</dev/tcp/127.0.0.1/8554' 2>/dev/null); then
	...
	echo "[cam2] Local RTSP proxy detected. Routing via proxy: $RTSP_URL"
else
	RTSP_URL="${CAM2_RTSP_URL:-rtsp://admin:password@192.168.0.x:554/stream1}"
	echo "[cam2] Local RTSP proxy not running. Routing directly: $RTSP_URL"
fi

AUDIO_FLAG=""
if [ "${CAM2_AUDIO:-off}" = "on" ]; then
	AUDIO_FLAG="--audio"
fi
